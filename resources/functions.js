...
module.exports = {
    ...
    contactsHTML: function(contactsData) {
        return new Promise(function(resolve, reject) {
            contactsHTML = ''
            for (var contactsElement in contactsData['data']) {
                contactsHTML += "<div class='col-lg-6 col-md-6 col-sm-6 col-xs-12 profile_details'>"
                contactsHTML += '<div class="well profile_view" style="display:block !important;overflow:auto">'
                contactsHTML += "<div class='col-sm-12'>"
                contactsHTML += "<h4 class='brief'><i>" + contactsData["data"][contactsElement]["title"] + "</i><button type='button' class='btn btn-primary btn-xs pull-right' onclick='displayData(" + JSON.stringify(contactsData['data'][contactsElement]) + ")' data-toggle='modal' data-target='#myModal'>Edit</button></h4>"
                ...
                contactsHTML += ' </div>'
                //contactsHTML += '<small class="right col-xs-2"><button type="button" class="btn btn-info btn-sm">Edit</button></small>'
        ...
        return new Promise(function(resolve, reject) {
            ...
            for ($i = 0; $i < 6; $i++) {
                ...
                for (var Element in $tasksData) {
                    ...
                    if (monthsLists[$i] == d.format('MMM YYYY')) {
                    	// console.log("$tasksData[Element]['id'] $tasksData[Element]['status'] : ", $tasksData[Element]['_id'], $tasksData[Element]['status'])
                    ...
                    if ($tasksData[Element]['dates']['type'] == "Planned") {
                        ...
                        if (!dueDate.isAfter(moment()) && $tasksData[Element]['status'] == "Active") {
                            // console.log("Risk Task FOund.", $tasksData[Element]['_id'], $tasksData[Element]['status'] )
            ...
            for (var taskindex in $tasksData) {
                ...
                if ((startDate.isAfter(moment()) && startDate.diff(moment(), 'days') <= 40) && $tasksData[taskindex]['status'] == "Active") {
                	// 5a632061196b670a9de4d9c5
                    isupcomingTask = true;
                    upcomingTaskHTML += "<tr><td style='text-align:center'><input type='checkbox' name='task' onclick='updateTaskStatus(" + JSON.stringify({ taskId: $tasksData[taskindex]['_id'], status: 'Completed' }) + ")'></td>";
                    upcomingTaskHTML += '<td style="text-align:center"><span class="fa fa-circle" style="color: Green;    font-size:18px;"></span></td>\
