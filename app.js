...
mongoose.connect('mongodb://localhost:27017/loginapp', { useMongoClient: true }); //missing variable "connection"?
                        ...
                        question_answer.getQuestionAnswers(taskID, function(err, QuestionAnswersData) {
                            roles.getallroles(function(roleErr, rolesDetails) {
                                ...
                                if (taskdetails) {
                                    ...
                                    var contactsDropdownHTML = '<option value="">Select</option>';
                                    for (var item in contactsContents['contactdata']['data']) {
                                        contactsDropdownHTML += '<option value="' + contactsContents['contactdata']['data'][item]['id'] + '"';
                                    ...
                                    var dependenciesDropdownHtml = '<option value="">Select</option>';
...
app.post('/task/update', AuthenteCheck.ensureAuthenticated, function(req, res) {
    ...
    if(req.body.title){
        taskentrydata["title"] = req.body.title;
    }
    if(req.body.description){
        taskentrydata["description"] = req.body.description;
    }
    if(req.body.roles){
        taskentrydata["roles"] = req.body.roles;
    }
    if(req.body.status){
        taskentrydata["status"] = req.body.status;
    }
    if(req.body.authorIds){
        taskentrydata["authorIds"] = req.body.authorIds;
    }
    if(req.body.dependencyIds){
        taskentrydata["dependencyIds"] = req.body.dependencyIds;
    }
    if(req.body.attachents){
        taskentrydata["attachents"] = req.body.attachents;
    }
...
app.get('/profile', AuthenteCheck.ensureAuthenticated, function(req, res) {
    var userDetails = req.user;
    console.log("profile user details : ", req.user);
    foldersModel.getfolders(function(folderserr, foldersContents) { //Get/Fetch folders
        functions.foldersHeierarcy((foldersContents)).then((foldersHeiraricalData) => {
...
app.post('/profile', AuthenteCheck.ensureAuthenticated, function(req, res) {
    ...
    var fname = "fname" in req.body ? req.body.fname : null;
    var lname = "lname" in req.body ? req.body.lname : null;
    var title = "title" in req.body ? req.body.title : null;
    var roles = "roles" in req.body ? req.body.roles : null;
    var type = "type" in req.body ? req.body.type : null;

    var userDetails = req.user;
    var updateData = {};

    email ? updateData["email"] = email : console.log("null fields found....");
    roles ? updateData["roles"] = roles : console.log("null fields found....");
    type ? updateData["type"] = type : console.log("null fields found....");
    fname ? updateData["firstname"] = fname : console.log("null fields found....");
    lname ? updateData["lastname"] = lname : console.log("null fields found....");
    title ? updateData["title"] = title : console.log("null fields found....");
...
app.post('/contacts', AuthenteCheck.ensureAuthenticated, function(req, res) {
    var id = req.body.id;
    var fname = req.body.fname;
    var lname = req.body.lname;
    var title = req.body.title;
    var type = req.body.type;
    var query = { 'contactdata.data': { $elemMatch: { 'id': id } } };
    contacts.query(query, function(err, contactData) {
        if (err) {
            return {
                message: err.message,
            };
        } else {
            let docId = contactData[0]._id;
            let dataObj = contactData[0].contactdata; //array
            dataObj.data.map(function(obj) {
                if (obj.id.toUpperCase() === id.toUpperCase()) {
                    obj.firstName = fname;
                    obj.lastName = lname;
                    obj.title = title;
                    obj.type = type;
            ...
            contacts.findOneAndUpdate({ '_id': docId }, { 'contactdata': dataObj }, function(err, userData) {
